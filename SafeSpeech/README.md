...
**选定** $$\mathcal{L}_{mel} = \|\hat{x}_{mel} - x'_{mel}\|_1$$ —— 因为所有 TTS 都输出语音，都能算 mel 频谱 ℓ1 距离，且收敛速度最快（实验中从 98.8 降到 16.7）。
...
>  **反直觉点**：$\mathcal{L}_{mel}$ 让合成接近"受保护音频"，$\mathcal{L}_{noise}$ 让合成接近"噪声"——看似冲突，实则**协同**：两者方向一致（都是"让模型学不到说话人特征"），只是机制不同。这就是为什么 β 在 5 个数量级范围内都有效——两个协同项的权重不敏感。
