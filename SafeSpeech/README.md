...
**区别于先前方法**：AntiFake / AttackVC 走对抗样本路线（$\max \mathcal{L}$）只能挡零样本克隆；SafeSpeech 走不可学习样本路线（$\min \mathcal{L}$），同时挡微调克隆和零样本克隆。
...
- (a) 能通过扰动 $\delta$ 优化（损失对 $\delta$ 可导，即可求梯度，且非零）
...
—— 因为所有 TTS 都输出语音，都能算 mel 频谱 $\ell_1$ 距离，且收敛速度最快（实验中从 98.8 降到 16.7）。

### 关键设计 2：SPEC（Speech PErturbative Concealment）

仅靠 $\mathcal{L}_{mel}$ 还会有残留的语音可懂度。作者引入"噪声引导"机制：
...
**反直觉点**：$\mathcal{L}\_{mel}$ 让合成接近"受保护音频"，$\mathcal{L}\_{noise}$ 让合成接近"噪声"——看似冲突，实则**协同**：两者方向一致（都是"让模型学不到说话人特征"），只是机制不同。这就是为什么 $\beta$ 在 5 个数量级范围内都有效——两个协同项的权重不敏感。

### 关键设计 3：感知优化

仅靠 $\ell_p$ 范数约束扰动幅度不足以保证人耳不可察觉。作者引入：
...
| 路线 | 数学 | 直觉 | 防御场景 |
|------|------|------|---------|
| **对抗样本**（AntiFake） | $\max \mathcal{L}$ | 让模型在推理时"考砸" | 零样本克隆 |
| **不可学习样本**（SafeSpeech） | $\min \mathcal{L}$ | 让模型以为自己"学会了"，提前停止学习 | 训练时克隆 + 零样本克隆 |
...
- "$\ell_p$ 范数约束在 4.1 节"——其实在 4 节开头的问题形式化里
